...
plt.legend()
plt.title("Средняя температура воздуха по регионам (Февраль–Апрель 2024)", fontsize=16, fontweight='bold')
plt.grid(True)
plt.xlabel("Дата")
...
def plot_region(region_code):
    ...
    plt.axhline(0, color='black', linestyle='--')
    plt.grid(True)
    plt.title(f"Температура в регионе {region_code}", fontsize=16, fontweight='bold')
    plt.xlabel("Дата")
    plt.ylabel("Температура, °C")
